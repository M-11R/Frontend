                ...
                <td colSpan={3} style={tdStyle}>
                  {participants.map((participant, index) => (
                    <div key={index} style={{ display: "flex", alignItems: "center", gap: "10px", marginBottom: "5px" }}>
                      <label>성명:</label>
                      <Field value={participant.name} setter={(val) => updateParticipant(index, "name", val)} />
                      <label>학번:</label>
                      <Field value={participant.studentId} setter={(val) => updateParticipant(index, "studentId", val)} />
...
const deleteButtonStyle: CSSProperties = { backgroundColor: "#f44336", color: "#fff", border: "none", padding: "8px 12px", cursor: "pointer", borderRadius: "5px", marginLeft: "15px" };
...
const Field = ({ label, value, setter, type = "text" }: { label?: string; value: string; setter: (value: string) => void; type?: string }) => (
  <div>
    ...
    <input type={type} value={value} onChange={(e) => setter(e.target.value)} style={{ width: "95%", padding: "10px", border: "1px solid #ccc", borderRadius: "5px" }} />
  </div>
);
